  ...
  function renderList() {
    ...
    for (var i = 0; i < system_names.length; i++) {
      ...
      if (subscribable) { 
       ...
       $("<div class='api-wrapper'>").text(name).append("<div class='card-body bg-catalog'><span>" + description + "</span><hr><span class='float-left small'><b>Owner: </b>"+supportEmail+ "</span><span class='float-right small'><b>Authentication: </b>"+ appsIdentifier +"</span></div>").append("<a class='card-footer text-white clearfix small z-1' href='"+ url +"' target='_blank'><span class='float-center'>Download JSON </span><span><img src='/images/"+ state +"' width='142' height='50'></span><span class='float-right'></span></a></div>").appendTo($("<a>", { "href": "?api=" + system_names[i] }).appendTo($(".api-list")));
    }
  }
